...
public class LinkPermissionsToRolesSeeder:IDataSeeder
{
    ...
    public async Task<DbRequest> SeedAsync(AppDbContext context)
    {
        try
        {
            if (await context.Permissions.AnyAsync(x=>x.Roles.Any()))
            {
                return DbRequest.Success("nothing to add");
            }
            ...
            if (getAdminRole == null || getManagerRole == null || getEmployeeRole == null || getObserverRole == null || getTeamLeaderRole == null)
            ...
            getObserverRole.Permissions = await context.Permissions.Where(x=>x.Name.ToLower().Contains("get")).ToListAsync();            
            getEmployeeRole.Permissions = await context.Permissions.Where(x=>x.Name.ToLower().Contains("get")).ToListAsync();            
