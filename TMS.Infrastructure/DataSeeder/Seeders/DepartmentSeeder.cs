...
public class DepartmentSeeder:IDataSeeder
{
    ...
    public async Task<DbRequest> SeedAsync(AppDbContext context)
    {
        ...
        try
        {
            ...
            var getTeamLeader = context.Users.Include(user => user.Employee).FirstOrDefault(x=>x.Roles.Any(y=>y.Name.ToLower()=="teamleader"));
            if(getTeamLeader is null) return DbRequest.Failure("TeamLeader not found");
            // foreach (var department in departmentsModel)
            // {
            var getEmployees = await context.Users.Where(x=>x.Roles.Any(y=>y.Name.ToLower()=="employee")).Select(x=>x.Employee).ToListAsync();
