...
public class UpdateWorkTaskService : IUpdateWorkTaskService
{
    private readonly IEntityCommiter _commiter;
    public UpdateWorkTaskService(IEntityCommiter commiter)
    {
        _commiter = commiter;
    }

    public async Task<ApiResponse> UpdateWorkTask(UpdateWorkTaskCommand request, CancellationToken cancellationToken)
    {
        if (request?.Dto == null)
            return ApiResponse.Failure(HttpStatusCode.BadRequest, "Invalid request.");

        var taskToUpdate = await _commiter.Tasks.GetAsync(
        ...
        var updateResult = await _commiter.Tasks.UpdateAsync(taskToUpdate.Data);
    ...
    private async Task<DbRequest<List<Project>>> GetProjects(List<int> projectsIds)
    {
        ...
        {
            var request = await _commiter.Projects.GetAsync(x => x.Id == id);
