...
public class SubmissionFilesZippingService : ISubmissionFilesZippingService
{
    private readonly IEntityCommiter _entityCommiter;
    private readonly ILogger<SubmissionFilesZippingService> _logger;
    public SubmissionFilesZippingService(IEntityCommiter entityCommiter, ILogger<SubmissionFilesZippingService> logger)
    {
        _entityCommiter = entityCommiter;
        _logger = logger;
    }

    public async Task<ApiResponse<ZipSubmissionFileResult>> GetAllSubmissionsFilesZip(GetAllSubmissionsFilesQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                using (var zipArchive = new System.IO.Compression.ZipArchive(memoryStream, System.IO.Compression.ZipArchiveMode.Create, true))
                {
                    ...
                    foreach (var file in allFiles)
                    {
                        ...
                        {
                            _logger.LogError(ex, $"Error adding file {file.OriginalFileName} to zip");
                        }
                    }
                }
                ...
                _logger.LogInformation($"Created and saved zip file at: {zipFilePath}");
